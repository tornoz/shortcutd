...
exports.parse = function (file) {
    ...
    for (var i = 0; i < config.shortcuts.length; i++) {
        ...
        for (var j in keys) {
            ...
            normalizedShortCut.code = inputCodes[keys[j]];
        }

        if (shortcut.value !== undefined) {
            normalizedShortCut.value = shortcut.value;
        } else {
            normalizedShortCut.value = 1;
        }
        normalizedShortCut.command = shortcut.command;
        normalizedShortCut.blocking = typeof shortcut.blocking !== 'undefined' 
            ? shortcut.blocking : false;

        normalizedShortcuts.push(normalizedShortCut);
    }
